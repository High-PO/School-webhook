...
exports.handler = async (event) => { // AWS Lambda Handler
    ...
    if (Today == not ){
        try{
        const rmqtlrx = await axios.post("<Your_WebHook_Url>", { // axios 모듈을 가지고 웹훅 주소로 post를 한다.
            "content":`${meal.month}월 ${meal.day}일 급식은 없거나 조회되지 않습니다.` //급식없는날 post
            });
            console.info("급식 없는 날"); // 콘솔에 띄운다.
        }
        catch(error){
            console.error("급식 조회불가 부분 오류"); // 급식 조회이 없는날 부분에서 오류 발생시 뜸
        }
    }
    else{
        try{ //시도
